...
def gui():
    ...
    seg_model = 'rim_substrate_v1'
    inDir = r'Z:\scratch\MayaMapper_Debug\mosaics'
    mosaicFileType = '.tif'
    outDirTop = r'Z:\scratch\MayaMapper_Debug'
    projName = 'maya_mapper_test'
    mapRast = True
    mapShp = True
    ...
    # MayaMapper
    if seg_model == 'rim_substrate_v1':
        modelDir = os.path.join(modelDir, seg_model)
    else:
        raise ValueError('seg_model not recognized: {}'.format(seg_model))

    from mayamapper.maya_mapper import do_work
    print('\n\nMapping habitat with MAYAMAPPER model...\n\n')
    do_work(
        inDir = inDir,
        outDirTop = outDirTop,
        projName = projName,
